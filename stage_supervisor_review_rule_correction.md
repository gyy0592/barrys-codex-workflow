Preparation correction only.

The specific workflow was updated while you were preparing files. Update the preparation files so every completed spec requires two parallel no-context reviews after each checkpoint commit:
...
Update only preparation files such as `control/goal.md`, `control/constraint.md`, and `workflow_20260612_physics_for_llm_scale_training/specs.md`. Do not start execution, do not start training, do not submit jobs, and do not start a controlled Codex. Stop after updating and verifying the prep files.
