...
grep -F 'This is the preparation phase only.' "$repo_dir/templates/prompt_for_run_prep.md" >/dev/null
...
grep -F 'waits for user review' "$repo_dir/README.md" >/dev/null
...
grep -F 'First commit any approved preparation files' "$repo_dir/templates/prompt_for_supervisor.md" >/dev/null
