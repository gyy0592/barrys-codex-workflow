...
## Current Internet Source Audit On 2026-07-07

| Search query | URL | Source title | Fact extracted | Local project check | Method or repair supported |
|---|---|---|---|---|---|
| `MathWorks trackingABF alpha beta filter constant velocity documentation` | https://www.mathworks.com/help/radar/ref/trackingabf.html | MathWorks `trackingABF` | The page describes alpha-beta tracking for linear motion, including constant velocity, with prediction and correction. | M01 uses only `x_screen`, prior commands, and internal state; it does not use simulator time or truth state. | Confirms the constant-velocity prediction idea used by M01 is still a valid internet-supported source. |
| `Astropy median absolute deviation robust statistics documentation` | https://docs.astropy.org/en/latest/stats/robust.html | Astropy Robust Statistical Estimators | MAD is defined as the median of absolute deviations from the median and supports robust rejection ideas. | M01 does not use MAD; this source remains support for later fixed methods such as M02 if M01 had not beaten A2. | Confirms the method-list source support remains recorded. |
| `FFmpeg freezedetect filter documentation repeated frozen frames` | https://ffmpeg.org/ffmpeg-filters.html | FFmpeg Filters Documentation | FFmpeg documents frozen-frame detection through low-change frame behavior. | The local algorithm sees scalar `x_screen`, so the usable adaptation is small compensated movement, not pixel comparison. | Confirms the unchanged-frame detection analogy used by M01. |
| `VPixx scientist guide frame dropping repeated previous image content` | https://docs.vpixx.com/vocal/a-scientist-s-guide-to-frame-dropping | VPixx A Scientist's Guide to Frame Dropping | The page says a dropped frame can repeat the previous frame's image content. | The local simulator can present repeated `t_frame` observations; offline checks compare predicted state with `t_frame` without feeding truth into M01. | Confirms separate recording of unchanged and suspected dropped-frame cases. |
| `MathWorks Smith Predictor delayed feedback internal model documentation` | https://www.mathworks.com/help/control/ug/control-of-processes-with-long-dead-time-the-smith-predictor.html | MathWorks Smith Predictor Example | The page describes using an internal model to predict delay-free response and comparing delayed output with delayed prediction. | PID is forbidden, so M01 uses only the compatible internal pending-command compensation idea, not PID control. | Confirms delayed-command compensation support. |
