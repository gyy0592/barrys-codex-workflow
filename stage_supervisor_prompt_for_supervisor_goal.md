...
- A correction issue means stop the bad run or bad path, record evidence, update the current spec or create the next fix spec, then continue. It does not mean the supervisor stops.
...
- Correct only when evidence proves drift, stalling, failed completion, or a required correction issue.
- If the executor is stable, monitor once every 2 to 10 minutes. Do not interrupt only because output is quiet.
- If the user adds a global requirement that changes execution permission, correction issues, required settings, or completion criteria, write it into the active control files before relying on it, then send a plain correction when needed and verify delivery.
