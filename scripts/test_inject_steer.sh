#!/usr/bin/env bash
#
# test_inject_steer.sh -- offline unit tests for inject_steer.sh.
#
...
#
# Run:  bash scripts/test_inject_steer.sh   (exit 0 = all pass)
...
# shellcheck source=inject_steer.sh
source "$HERE/inject_steer.sh"
...
MOCK_INPUT=""
_input_region() { printf '%s' "$MOCK_INPUT"; }
