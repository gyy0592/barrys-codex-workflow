#!/usr/bin/env bash
#
# inject_steer.sh -- robust, mechanical message injection into a Codex CLI TUI
...
#   STEER_EVIDENCE_DIR    base dir for evidence (default ./temp/tmux-codex-supervisor)
...
usage() {
  err "usage: inject_steer.sh {verify-target|type|submit|send} <pane> [file]"
  exit 64
}
...
EVID=""
_evidence_dir() {
  local base="${STEER_EVIDENCE_DIR:-./temp/tmux-codex-supervisor}"
...
EOF
  ...
  # ever landing in a plain shell or another non-Codex pane.
